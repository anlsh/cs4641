...
def random_hill_climb(problem, max_attempts=10, max_iters=np.inf, restarts=0,
    ...
    best_fitness: float
        Value of fitness function at best state.
    history: array
        numpy array of fitness function at each iteration of the algorithm
    ...
    history_queue = collections.deque()
    for _ in range(restarts + 1):
        ...
        attempts = 0
        iters = 0
        current_fitness = problem.eval_fitness(problem.state)

        while (attempts < max_attempts) and (iters < max_iters):
            iters += 1
            history_queue.append(current_fitness)
    ...
    best_fitness = problem.get_maximize()*best_fitness
    return best_state, best_fitness, np.array(list(history_queue))
...
def simulated_annealing(problem, schedule=GeomDecay(), max_attempts=10,
    ...
    Returns
    -------
    best_state, best_fitness, history
